...
SKIPPED_IMAGES_LOG = os.path.join(OUTPUT_PATH, "skipped_images.txt")
...
def preprocess_data():
    ...
    image_files = sorted([f for f in os.listdir(IMAGE_PATH) if os.path.splitext(f)[1].lower() in VALID_EXTENSIONS])
    print(f"📂 Found {len(image_files)} images. Processing in batches...")
    
    open(SKIPPED_IMAGES_LOG, "w").close()  # Clear previous logs
    ...
    for i in range(0, len(image_files), BATCH_SIZE):
        ...
        for img_name in tqdm(batch_files, desc=f"Processing Batch {i//BATCH_SIZE + 1}"):
            ...
            if img is None:
                print(f"⚠️ Warning: Unable to load image {img_name}. Skipping...")
                with open(SKIPPED_IMAGES_LOG, "a") as log_file:
                    log_file.write(f"{img_name}\n")  # Log skipped image
                continue  # Skip this image and move to the next one

            img = cv2.resize(img, IMAGE_SIZE)  # Resize
            img = img / 255.0  # Normalize
            img = np.expand_dims(img, axis=-1)  # Ensure (224, 224, 1)
